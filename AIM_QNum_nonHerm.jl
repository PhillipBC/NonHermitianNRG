...
function get_LR_eig(A::Matrix)
    ...
    # Returns the eigenvalues, left eigenvectors, right eigenvectors, the maximum residual, and the deviation from bi-orthonormality
    ...
    for (t, e) in enumerate(evals)
        # Evaluate ||H|E⟩ - E|E⟩||
        R_r = norm(A * Ur[:, t] - e * Ur[:, t])
        # keep track of the largest residual
        if R_r > max_r
            max_r = R_r
        end
        L_r = norm(A' * Ul[:, t] - e' * Ul[:, t])
        if L_r > max_r
            max_r = L_r
        end
    end
    ...
    Id = diagm(ones(length(evals))) # identity matrix
...
function load_pvals(arg) 
    ...
    # System params
    U = parse(ComplexF64, pvals[5])            # Spin-spin interaction 
    eps = parse(ComplexF64, pvals[6])          # Energy of spin
    V = parse(ComplexF64, pvals[7])            # impurity coupling
    magfield = Float64(pvals[8])               # Magnetic field strength 
    ...
    disorder = pvals[12]                # disorder strength (0.0 for no disorder)

    return lmax, rlim, lambda, elim, U, eps, V, magfield, gamma, n_pots, sort_type, disorder
end 

do_load = false

if do_load
    lmax, rlim, lambda, elim, U, eps, V, magfield, gamma, n_pots, sort_type, disorder = load_pvals(0)
else 
    # NRG params
    ...
    elim = 1e20       # Maximum allowed eigenvalue during truncation  

    # System params  
    U = 0.3 + 0im         # Spin-spin iteraction
    eps = -0.15 - 0.0im     # Energy of spin
    V = 0.08 - 0.05im        # impurity coupling
    ...
    disorder = 0.0 # disorder strength (0.0 for no disorder)
end
...
function get_wilson_params_imag(lambda::Number, lmax::Number, gamma::Number, n_pots::Number, V::Number)
    # Get parameters for the wilson chain mapping, want to decrease with growing length
    # Be careful with convention!!
    tn = zeros(ComplexF64, lmax + 2)  # -1,0,1,2,....,lmax-1,lmax (hopping coeffs)
    ...
    if n_pots > 0
        ...
        if n_pots < lmax
            en[nse] = (1.0im * gamma) .* tn[nse] 
        else
            en[2:lmax+2] = (1.0im * gamma) .* tn[2:lmax+2]  
        end
    end
        ...
        for (jk, k) in enumerate(ks) # loop over possible states 
            ...
            for s in eachindex(sigs)
                if !iszero(M[s, jk, jkp])
                    ...
                    for r in 1:rmaxofk[l+2, qnind1, jk], rp in 1:rmaxofk[l+2, qnind1, jkp] 
                        H[rstartk[l+2, qnind1, jk]+r, rstartk[l+2, qnind1, jkp]+rp] +=
                            tn[l+1] * M[s, jk, jkp] * ((-1)^k) * UM[s, qnind2][r, rp] 
                    end # (r,rp) loop
                ...
                if !iszero(M[s, jkp, jk])
                    ...
                    for r in 1:rmaxofk[l+2, qnind1, jk], rp in 1:rmaxofk[l+2, qnind1, jkp]
                        H[rstartk[l+2, qnind1, jk]+r, rstartk[l+2, qnind1, jkp]+rp] +=
                            tn[l+1] * M[s, jkp, jk] * ((-1)^kp) * UMd[s, qnind2][r, rp] 
                    end # (r,rp) loop
                end # non-zero M element
...
function save_data_AIM_NonHerm(lmax, rlim, lambda, elim, p_U, p_eps, V, gamma, disorder, energies, diffs, QN, rkept, biorths)
    loc = "/Data/AIM/eval_flow"
    p_U = replace("$(p_U)", " " => "")
    p_eps = replace("$(p_eps)", " " => "")
    p_V = replace("$(V)", " " => "")
    fname = "n$(lmax)_r$(rlim)_Lam$(lambda)_elim$(elim)_U$(p_U)_eps$(p_eps)_V$(p_V)_en_is_tn_gam$(gamma)_n$(n_pots)_eta($disorder)"
    if qsym == 1 && szsym == 1
        ...
        fstring = "$(loc)/Q_Sz_conserved/$(sort_type)/NHAIM_biorths_$(fname)"
        fstring = "." * replace(fstring, "." => "_") * ".jld2"
    ...
    elseif szsym == 1 && qsym != 1
        ...
        fstring = "$(loc)/Sz_conserved/$(sort_type)/NHAIM_biorths_$(fname)"
        fstring = "." * replace(fstring, "." => "_") * ".jld2"
        save(fstring, "biorths", biorths)
    ...
    elseif szsym != 1 && qsym == 1
        ...
        fstring = "$(loc)/Q_conserved/$(sort_type)/NHAIM_biorths_$(fname)"
        fstring = "." * replace(fstring, "." => "_") * ".jld2"
        save(fstring, "biorths", biorths)
        ...
        fstring = "$(loc)/Q_Sz_nonconserved/$(sort_type)/NHAIM_biorths_$(fname)"
        fstring = "." * replace(fstring, "." => "_") * ".jld2"
        save(fstring, "biorths", biorths)
...
tn, en = get_wilson_params_imag(lambda, lmax, gamma, n_pots, V); # for flat band
...
function iterative_loop_NonHerm(rmax::Array, rkept::Array, UM::Array, UMd::Array, energies::Array, eground::Array, QN::Dict, iter_count::Array, numqns::Number, disorder::Number)
    ...
    for l in 0:lmax
        ...
        for (q, sz, qnind1) in [(k[2], k[3], v) for (k, v) in QN if (k[1] == l + 2)] # find all the keys in QN at iteration l
            ...
            # Store the eigenvalues in the energies array, and the eigenvectors in hl and hr, and the residuals/biorthogonalization measure in diffs and biorths
            energies[l+2, qnind1][1:d], hl[qnind1][1:d, 1:d], hr[qnind1][1:d, 1:d], diffs[qnind1, l+2], biorths[qnind1, l+2] = get_LR_eig(H)
    ...
    save_data_AIM_NonHerm(lmax, rlim, lambda, elim, p_U, p_eps, V, gamma, disorder, energies, diffs, QN, rkept, biorths)
...
function plot_lowest_AIM_flow(energies::Array, rkept::Array, show_title::Bool=true)
    ...
    n_vals = 128 # manuall set number of states to plot, or set to md to plot all states
    msz = 2.5
    lw = 0.01
    period = 2
    starter = 1
    ...
    if show_title
        ...
        ax2.set_title("\$V = $V\$ -  ($rlim kept)", fontsize=14)
    end
...
flow = plot_lowest_AIM_flow(energies, rkept, true); 
